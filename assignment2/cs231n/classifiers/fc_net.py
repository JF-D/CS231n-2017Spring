...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            loss += 0.5 * self.reg * np.sum(self.params['W%d' % (j+1)] ** 2)
            if self.use_dropout:
                dout = dropout_backward(dout, dp_cache[j])
            if self.use_batchnorm:
                dout, dw, db, dgamma, dbeta = affine_bn_relu_backward(dout, fc_cache[j])
                grads['gamma%d' % (j+1)], grads['beta%d' % (j+1)] = dgamma, dbeta
            else:
                dout, dw, db = affine_relu_backward(dout, fc_cache[j])
